...
const DUMMY_DATA = [
  {
    title:
      'how to make chicken soup - dinner recipes - homemade from scratch - stock recipes -',
  ...
  if (showVideo === false) {
    ...
    return (
      <VideoPlayerContainer>
        ...
        ></iframe>
        <StyledButton onClick={() => setShowVideo(false)}>
          Close Video
        </StyledButton>
        {authState && (
          <StyledButton onClick={() => addItem(videoId, videoTitle)}>
            Add To Favorites
          </StyledButton>
        )}
      </VideoPlayerContainer>
